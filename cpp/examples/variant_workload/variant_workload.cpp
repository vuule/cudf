...
// Define VARIANT_LOOPED_ONLY to build against a libcudf without `extract_variant_fields` (i.e.
// main).
...
int main(int argc, char const** argv)
{
  ...
  std::cout << "api        : "
            << (std::getenv("VARIANT_WORKLOAD_BATCHED") != nullptr &&
                    std::string_view{std::getenv("VARIANT_WORKLOAD_BATCHED")} == "1"
                  ? "batched"
                  : "looped")
            << "\n\n";
  ...
  // Paths grouped by the column they read, so the batched API can take each column in one call.
  std::array<std::vector<std::string_view>, 3> paths_by_col;
  for (auto const& entry : kWorkloadPaths) {
    paths_by_col[static_cast<int>(entry.col)].emplace_back(entry.path);
  }

  bool const batched = [] {
    auto const* setting = std::getenv("VARIANT_WORKLOAD_BATCHED");
  ...
  auto run_once = [&]() {
    ...
    if (batched) {
#ifdef VARIANT_LOOPED_ONLY
      // Built against a libcudf without the batched API, so this configuration is unavailable.
      CUDF_FAIL("batched extraction not available in this build");
#else
      for (int col = 0; col < 3; ++col) {
        auto const& paths = paths_by_col[col];
        if (paths.empty()) { continue; }
        std::vector<cudf::data_type> const types(paths.size(), string_type);
        auto result = cudf::io::parquet::experimental::extract_variant_fields(
          col_views[col], paths, types, {}, stream, mr);
      }
#endif
    } else {
      for (auto const& entry : kWorkloadPaths) {
        auto result = cudf::io::parquet::experimental::extract_variant_field(
          col_views[static_cast<int>(entry.col)],
