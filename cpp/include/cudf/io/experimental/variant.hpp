 ...
 * empty nor one column per path, or any of its columns is nullable, not `UINT8`, or has a different
 ...
 * @param statuses Optional. When non-empty, one status column per path, each filled with that
 * path's `variant_operation_status` values, one per row, as `extract_variant_field` fills its own.
 * Each must be non-nullable, `UINT8`, and have the same row count as `variant_column`. Any incoming
