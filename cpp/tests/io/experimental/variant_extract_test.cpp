                               ...
                               std::vector<std::string> const& paths)
{
  ...
  std::vector<std::unique_ptr<cudf::column>> status_columns;
  std::vector<cudf::mutable_column_view> statuses;
  status_columns.reserve(paths.size());
  statuses.reserve(paths.size());
  for (std::size_t p = 0; p < paths.size(); ++p) {
    status_columns.push_back(make_status_buffer(variant.size()));
    statuses.push_back(status_columns.back()->mutable_view());
  }

  auto const got =
    cudf::io::parquet::experimental::get_variant_fields(variant, path_views, statuses, stream);
  ...
  for (std::size_t p = 0; p < paths.size(); ++p) {
    ...
    CUDF_TEST_EXPECT_COLUMNS_EQUAL(*status_columns[p], *expected_status);
  }
}
...
TEST_F(GetVariantFieldStatusTest, MixedRows)
{
  ...
  // Row 2: {q: INT32(0)} (no x key) → missing_path. The dictionary has to declare the key the
  // object's field id refers to; an id with no dictionary entry is malformed, not a missing key.
  auto const m2 = build_metadata({"q"});
  auto const v2 = build_single_field_object(/*fid=*/0, enc_int32(0));
...
TEST_F(GetVariantFieldStatusTest, FieldIdOutsideDictionaryIsMalformed)
{
  // An object field whose id has no dictionary entry cannot be named, so the object is malformed
  // rather than simply missing the requested key.
  auto const m   = build_metadata({});
  auto const v   = build_single_field_object(/*fid=*/0, enc_int32(7));
  auto const col = wrap_single_variant(m, v);

  auto status = make_status_buffer(cudf::column_view{col}.size());
  auto got    = cudf::io::parquet::experimental::get_variant_field(
    col, "x", status->mutable_view(), cudf::test::get_default_stream(), cmr());
