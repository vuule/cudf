...
struct t_array_stats {
  ...
  using length_type = decltype(std::declval<ReturnType>().size_bytes());
  length_type length{};  //!< length of data
  ...
  __host__ __device__ operator ReturnType() const { return {ptr, length}; }
};
