      ...
      case dtype_timestamp64:
        ...
        // }
        {
          ...
          if (s->chunk.has_minmax) {
            ...
            // Both pairs hold the writer timezone's wall clock, as Apache writes them: the values
            // a reader materializes from the re-based data stream. The shift is applied to each
            // timestamp before the extrema are reduced, in `gather_statistic_blobs`.
